...
\begin{document}
\title{DataONE RunManager: a scientific workflow and provenance data management software}
...
\begin{abstract}
In this demo, we demonstrate a scientific workflow and provenance data management software -- DataONE RunManager that  can capture, store, query, visualize, and publish of a single Matlab script or multiple Matlab script runs. The important features of DataONE RunManager software include: (1) there are three forms of data provenance supported by RunManager: prospective provenance, retrospective provenance, and hybrid provenance; (2) for a workflow project, we have multiple provenance graphs consisting of a graph of prospective provenance, a graph of hybrid provenance, a graph of retrospective multiple\_scripts\_multiple\_runs provenance. 
\end{abstract}
...
The DataONE RunManager is a scientific workflow and provenance data management software. It can capture, store, query, visualize, and publish of one or multiple Matlab or R script runs. There are three types of provenance supported by RunManager: prospective provenance, retrospective provenance, and hybrid provenance. The prospective provenance is expressed using YW tags. The retrospective provenance captured during a script execution includes information about the script that was run, files that were read or written, and details about the execution environment at the time of execution.
...
RunManagers can transparenly capture retrospective provenance from MATLAB and R script executions. DataONE RunManagers overload and thus intercept file I/O operations to automatically capture runtime observables at the file level. DataONE provides two implementations: the \texttt{recordr} R package and the \texttt{matlab-dataone} toolbox. The provenance captured during a script execution includes information about the script that was run, the files that were read or written, and details about the execution environment at the time of execution.

DataONE RunManagers support the following APIs: \texttt{record},  \texttt{startRecord}, \texttt{endRecord}, \texttt{listRuns}, \texttt{deleteRuns}, \texttt{viewRun(s)}, \texttt{traceRuns} for R, \texttt{plotRuns} for R and \texttt{publishRun} in order to capture, search, archive, visualize, and reproduce a script run. The captured retrospective provenance are populated to a relational provenance database which has these tables for \texttt{execution metadata}, \texttt{file metadata}, \texttt{tag}, \texttt{module dependencies} for MATLAB and \texttt{provenance relationships} for R. Meanwhile, the captured retrospective provenance can be exported as provenance facts that can be used by logical rules. Retrospective provenance graphs can be rendered from the exported provenance facts. Then, a multiple\_scripts\_multiple\_runs provenance graph can be produced by executing a Prolog query \footnote{\label{query1}\url{https://github.com/yesworkflow-org/yw-idcc-17/blob/master/OHIBC\_Howe\_Sound\_project/multi\_runs\_retrospective\_provenance\_queries/queries/render\_rm\_graph\_upstream\_of\_file\_q1\_sha256.sh}} which consists of four Prolog queries using the exported provenance facts. A example set of logical rules are highlighted in List~\ref{logic_rule_multi_prov}. The joining operation is based on file content (hash code). For the recursive rules \texttt{rm\_file\_downstream} computes all upstream files for the given file \texttt{Sha256\_2}. It has two base cases: one is the file object is itself downstream file object and the other is immediate derived relationship.  
...
\begin{figure}
  \centering
  \subfloat[prospective provenance graph]{\includegraphics[width=\columnwidth, height=3.3in]{figures/OHI_complete_wf_graph_uri}} \\
  \subfloat[hybrid provenance graph]{\includegraphics[width=\columnwidth, height=3.3in]{figures/OHI_recon_complete_graph_all_observables}}
...
\begin{figure}
  \includegraphics[height=3.2in, width=0.9\columnwidth]{figures/trace}
...
DataONE allows to make experiments reproducible by providing two capabilities. First, as we have demonstrated above, DataONE RunManagers are able to capture both prospective and retrospective provenance. Second, DataONE data repository allows users to package workflow results (implemented in any programming languages such as Python, R, MATLAB, SAS, etc.) in a structured DataONE data package and publish to a member node inside a DataONE federation network so that the workflow results (including 2D graphs, 3D graphs, scripts, data, provenance relationship, science metadata describing the experimental methods, authors, geographical coverage,  time coverage provided by data providers, etc.) can be displayed on DataONE data repository and be shared or linked with other scientific experiments in a larger scale. It is different from providing reproducibility based on IPython notebook [ref] or a virtual machine, such as a Docker or a VM. DataONE package contains provenance information (\texttt{prov:used}, \texttt{prov:wasGeneratedBy}, or \texttt{prov:wasDerivedFrom}) to describe the relationships between digital objects and datapackages for reproducibility, instead of only encapsulating digital objects. 
...
With our DataONE RunManager demo, we showcase how RunManager can capture, store, query, visualize, and publish prospective, retrospective and hybrid provenance. Moreover, we demonstrated that provenance queries across heterogeneous provenance sources can yield a more complete and comprehensible picture of data provenance. We also think provenance describing datasets can facilitate data discovery for others. 


\bibliographystyle{ACM-Reference-Format}
\bibliography{ssdbm_ref} 
